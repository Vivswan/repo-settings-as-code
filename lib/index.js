...
function writeSummary(outcomes, mode) {
  ...
  for (const outcome of outcomes) {
    const detail = outcome.detail.map((line) => line.replace(/\\/g, "\\\\").replace(/\|/g, "\\|").replace(/\r?\n/g, " ")).join("<br>") || "-";
...
async function run() {
  ...
  if (typeof settings !== "object" || Array.isArray(settings)) {
    return fail(`${settingsFile} must be a YAML mapping of section names to settings, but its top level parsed as ${Array.isArray(settings) ? "a list" : `a ${typeof settings}`}. Rewrite the top level as "section: ..." keys`);
  }
  ...
  if (unknownKeys.length > 0) {
    if (onlySections.size > 0 && unknownKeys.every((key) => !onlySections.has(key))) {
      annotate("warning", `ignoring unknown top-level section(s) outside the "sections" allowlist: ${unknownKeys.join(", ")}. Upgrade the action to a version that knows them, or remove them from ${settingsFile}`);
    } else {
      return fail(`unknown top-level section(s) in ${settingsFile}: ${unknownKeys.join(", ")} (known: ${SECTION_KEYS.join(", ")}). Fix the typo, or prefix private keys with "_", or set the "sections" input to limit processing`);
    }
  }
  ...
  if (!ctx.check && onMissingPermission === "fail") {
    ...
    if (denied.length > 0) {
      ...
      return fail(`preflight failed: the token cannot access ${denied.length} section(s), so nothing was applied. Grant the permissions named above, or set on-missing-permission: warn to skip those sections`);
    }
  }
  const outcomes = [];
  let failed = false;
  ...
  for (const section of SECTIONS) {
    ...
    try {
      ...
      if (error instanceof PermissionDenied) {
        ...
        annotate("error", `${section.key}: not applied${required ? " (listed in required-sections, so this fails the run)" : ""} - ${error.detail}`);
  ...
  setOutput("skipped-sections", outcomes.filter((o) => o.status === "skipped").map((o) => o.key).join(","));
  if (failed) {
    setOutput("result", "failed");
    return 1;
  }
  if (ctx.check) {
    const result2 = drifted ? "drift" : partial ? "partial" : "clean";
    setOutput("result", result2);
