        ...
        if ("error" in probe) {
          ...
          for (const key of REQUIRED_PROTECTION_KEYS) {
            if (!(key in branch.protection) && live[key] != null && live[key] !== false) {
              result.drift.push(`branches[${branch.name}].protection.${key}: set live but omitted from the declaration - apply would REMOVE it`);
            }
          }
        }
      } else {
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const label of desired) {
      ...
      if (label.new_name && bySource && byTarget && bySource !== byTarget) {
        throw new Error(`labels: cannot rename "${label.name}" to "${finalName}" - both exist as separate labels; delete one first`);
      }
      ...
      if (colorDrift || descriptionDrift || renameDrift || extraDrift.length > 0) {
        if (ctx.check) {
          ...
          if (colorDrift) {
            result.drift.push(`labels[${finalName}].color: "${wantColor}" != "${normalizeColor(existing.color)}"`);
          }
          if (descriptionDrift) {
            result.drift.push(`labels[${finalName}].description: ${JSON.stringify(wantDescription)} != ${JSON.stringify(existing.description ?? "")}`);
          }
...
var autolinksSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const autolink of desired) {
      ...
      if (ctx.check) {
        result.drift.push(existing ? `autolinks[${autolink.key_prefix}]: differs (autolinks are immutable; recreate)` : `autolinks[${autolink.key_prefix}]: missing`);
        continue;
      }
      if (existing) {
...
var actionsSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    if (routed.length > 0) {
      result.notes.push(`unknown key(s) [${routed.join(", ")}] passed through to /actions/permissions - run check mode to confirm they took effect (a future /workflow-endpoint field would belong under the known workflow keys)`);
    }
    if (ctx.check) {
...
var collaboratorsSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const collaborator of desired) {
      ...
      if (ctx.check) {
        result.drift.push(existing ? `collaborators[${collaborator.username}]: role "${existing.role_name}" != wanted "${wantRole}"` : `collaborators[${collaborator.username}]: missing (invitation would be sent)`);
      } else {
    ...
    if (ctx.check) {
      ...
      if ("enable_vulnerability_alerts" in desired) {
        ...
        if (enabled !== Boolean(desired.enable_vulnerability_alerts)) {
          result.drift.push(`repository.enable_vulnerability_alerts: ${desired.enable_vulnerability_alerts} != ${enabled}`);
        }
      }
      if ("enable_automated_security_fixes" in desired) {
        ...
        if (enabled !== Boolean(desired.enable_automated_security_fixes)) {
          result.drift.push(`repository.enable_automated_security_fixes: ${desired.enable_automated_security_fixes} != ${enabled}`);
        }
      }
      return result;
    }
...
async function run() {
  ...
  if (unknownKeys.length > 0) {
    if (onlySections.size > 0 && unknownKeys.every((key) => !onlySections.has(key))) {
      annotate("warning", `ignoring unknown top-level section(s) outside the sections allowlist: ${unknownKeys.join(", ")}`);
    } else {
  ...
  if (!ctx.check && onMissingPermission === "fail") {
    ...
    if (denied.length > 0) {
      ...
      return fail(`preflight failed for ${denied.length} section(s); nothing was applied (fix the token or use on-missing-permission: warn)`);
    }
  }
  const outcomes = [];
  let failed = false;
  ...
  for (const section of SECTIONS) {
    ...
    try {
      ...
      if (error instanceof PermissionDenied) {
        ...
        const hint = error.detail.includes(" 404 ") ? "token lacks permission or the resource does not exist" : "token lacks permission";
        annotate("error", `${section.key}: ${hint}${required ? " (required section)" : ""} - ${error.detail}`);
