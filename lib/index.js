...
class GithubApi {
  ...
  baseUrl;
  apiVersion;
  constructor(token, baseUrl = process.env.GITHUB_API_URL ?? "https://api.github.com", apiVersion = "2022-11-28") {
    this.token = token;
  ...
  async request(method, path, payload) {
    ...
    if ("error" in result) {
      ...
      throw new Error(`${method} ${path} failed: ${status} ${message}. Check the token's permissions and the request payload against the GitHub REST docs for this endpoint`);
    }
    return result.data;
  }
  async tryRequest(method, path, payload, options) {
    ...
    debugLog(`${method} ${path} -> ${response.status} (${Date.now() - started}ms)` + (payload === undefined ? "" : ` payload: ${JSON.stringify(payload)}`));
    if (!response.ok) {
      let message = text;
      try {
  ...
  async getRepoFile(slug, filePath) {
  ...
  async list(path) {
    ...
    for (let page = 1;; page++) {
      ...
      if (!Array.isArray(chunk)) {
        throw new Error(`GET ${path} returned a JSON value that is not a list, so the response cannot be paginated. Check that the path is a list endpoint and the API version header matches the GitHub REST docs`);
      }
      items.push(...chunk);
      if (chunk.length < 100) {
        return items;
...
function applyDefaults(defaults, repoSettings) {
  const merged = deepMerge(defaults, repoSettings);
  const disabled = [];
...
function subsetDiff(desired, live, path) {
  ...
  if (typeof desired === "object") {
    ...
    for (const [key, value] of Object.entries(desired)) {
      drift.push(...subsetDiff(value, live[key], `${path}.${key}`));
    }
    return drift;
  }
...
function diffArray(desired, live, path) {
  if (!Array.isArray(live)) {
    return [`${path}: expected list, live has ${JSON.stringify(live)}`];
  }
  const desiredTypes = desired.map((item) => typeof item === "object" && item !== null && ("type" in item) ? String(item.type) : null);
  const liveTypes = Array.isArray(live) ? live.map((item) => typeof item === "object" && item !== null && ("type" in item) ? String(item.type) : null) : [];
  const typed = desired.length > 0 && desiredTypes.every((t) => t !== null) && new Set(desiredTypes).size === desiredTypes.length && liveTypes.every((t) => t !== null) && new Set(liveTypes).size === liveTypes.length;
  if (typed) {
    const drift2 = [];
    const liveByType = new Map;
    for (const item of live) {
      if (typeof item === "object" && item !== null && "type" in item) {
        liveByType.set(String(item.type), item);
      }
    }
    ...
    for (const item of desired) {
      ...
      if (match === undefined) {
        drift2.push(`${path}[${type}]: missing live`);
      } else {
        drift2.push(...subsetDiff(item, match, `${path}[${type}]`));
      }
    }
    for (const type of liveByType.keys()) {
      if (!declaredTypes.has(type)) {
        drift2.push(`${path}[${type}]: present live but not declared`);
      }
    }
    return drift2;
  }
  const objectList = desired.length > 0 && desired.every((item) => typeof item === "object" && item !== null && !Array.isArray(item));
  if (objectList) {
    const drift2 = [];
    const liveItems = [...live];
    for (const [index, item] of desired.entries()) {
      const matchIndex = liveItems.findIndex((candidate) => subsetDiff(item, candidate, "").length === 0);
      if (matchIndex === -1) {
        drift2.push(`${path}[${index}]: no matching live entry for ${JSON.stringify(item)}`);
      } else {
        liveItems.splice(matchIndex, 1);
      }
    }
    for (const leftover of liveItems) {
      drift2.push(`${path}: live entry not declared: ${JSON.stringify(leftover)}`);
    }
    return drift2;
  }
...
async function call(ctx, section, method, path, payload) {
...
async function listAll(ctx, section, path) {
  const items = [];
  const separator = path.includes("?") ? "&" : "?";
  for (let page = 1;; page++) {
    ...
    if (!Array.isArray(chunk)) {
      throw new Error(`${section}: GET ${path} returned a JSON value that is not a list, so the response cannot be paginated. Check the "api-version" input against the GitHub REST docs for this endpoint`);
    }
    items.push(...chunk);
    if (chunk.length < 100) {
      return items;
...
function throwFor(section, method, path, error) {
  ...
  if (isPermissionError(error)) {
    ...
    const grant = section === "teams" ? `grant "Members" (read) under the PAT's Organization permissions and "Administration" (read and write) under its Repository permissions` : `grant "${SECTION_PERMISSION[section] ?? "Administration"}" (read and write) under the PAT's Repository permissions`;
    throw new PermissionDenied(section, `the token was denied ${cause}${alsoMissing}. To fix, ${grant}`);
  }
  if (error.status >= 500) {
    throw new Error(`${section}: ${cause}. GitHub returned a server error; re-run the workflow, and retry later if it persists`);
  }
  if (error.status === 401) {
    throw new Error(`${section}: ${cause}. The token was rejected as invalid or expired; update the token input (or the secret it reads) with a valid, unexpired PAT`);
  }
  if (error.status === 429) {
    throw new Error(`${section}: ${cause}. The API rate limit was hit; re-run the workflow after the limit resets, or use a token with a higher rate limit`);
  }
  throw new Error(`${section}: ${cause}. The API rejected the request; fix the "${section}" values in the settings file to satisfy the message above`);
}

// src/sections/branches.ts
var REQUIRED_PROTECTION_KEYS = [
...
var branchesSection = {
  key: "branches",
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    for (const branch of desiredRaw) {
      ...
      if (branch.protection === null) {
        ...
        if (ctx.check) {
          if (isProtected) {
            result.drift.push(`branches[${branch.name}]: protected live but the settings file declares protection: null; apply will remove the protection`);
          }
        } else if (isProtected) {
      ...
      const payload = { ...branch.protection };
      ...
      if (ctx.check) {
        ...
        if ("error" in probe) {
          ...
          if ("error" in branchProbe && branchProbe.error.status === 404) {
            result.drift.push(`branches[${branch.name}]: declared in the settings file but the branch does not exist on the repo, so apply cannot protect it; create the branch, or remove it from the settings file`);
          } else {
            result.drift.push(`branches[${branch.name}]: unprotected live but the settings file declares protection; apply will protect it`);
          }
        } else {
          const live = flattenProtection(probe.data);
          result.drift.push(...subsetDiff(branch.protection, live, `branches[${branch.name}].protection`));
          for (const key of REQUIRED_PROTECTION_KEYS) {
            if (!(key in branch.protection) && live[key] != null && live[key] !== false) {
              result.drift.push(`branches[${branch.name}].protection.${key}: set live but omitted from the settings file, so apply would REMOVE it; add ${key} to the branch's protection in the settings file to keep it`);
            }
          }
        }
      } else {
...
var ACTOR_NAME_KEYS = ["login", "slug"];
var ACTOR_LIST_KEYS = new Set(["users", "teams", "apps"]);
function flattenValue(value) {
  ...
  if ("enabled" in record && typeof record.enabled === "boolean" && keys.every((k) => k === "enabled" || k === "url" || k.endsWith("_url"))) {
  ...
  for (const [key, inner] of Object.entries(record)) {
    ...
    } else if (key.endsWith("_url") || key === "url") {} else {
...
var labelsSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    const live = await listAll(ctx, this.key, `/repos/${ctx.repo}/labels`);
    ...
    for (const label of desired) {
      ...
      if (label.new_name && bySource && byTarget && bySource !== byTarget) {
        throw new Error(`labels: cannot rename "${label.name}" to "${finalName}" - both already exist as separate labels on the repo; delete one of them on GitHub, or remove new_name from "${label.name}" in the settings file`);
      }
      ...
      delete extraKeys.color;
      delete extraKeys.description;
      if (!existing) {
        if (ctx.check) {
          result.drift.push(`labels[${finalName}]: missing - declared in the settings file but not on the repo; apply will create it`);
        } else {
      ...
      const descriptionDrift = label.description !== undefined && (existing.description ?? "") !== wantDescription;
      ...
      if (colorDrift || descriptionDrift || renameDrift || extraDrift.length > 0) {
        if (ctx.check) {
          if (renameDrift) {
            result.drift.push(`labels[${existing.name}]: should be named "${finalName}" per the settings file; apply will rename it`);
          }
          if (colorDrift) {
            result.drift.push(`labels[${finalName}].color: declared "${wantColor}" != live "${normalizeColor(existing.color)}"; apply will set the declared value`);
          }
          if (descriptionDrift) {
            result.drift.push(`labels[${finalName}].description: declared ${JSON.stringify(wantDescription)} != live ${JSON.stringify(existing.description ?? "")}; apply will set the declared value`);
          }
          result.drift.push(...extraDrift);
        } else {
          await call(ctx, this.key, "PATCH", `/repos/${ctx.repo}/labels/${encodeURIComponent(existing.name)}`, {
            new_name: finalName,
            ...wantColor === undefined ? {} : { color: wantColor },
    ...
    for (const label of liveByKey.values()) {
      if (!declaredKeys.has(nameKey(label.name))) {
        if (ctx.check) {
          result.drift.push(`labels[${label.name}]: undeclared - not in the settings file, so apply will DELETE it; add it to the settings file to keep it`);
        } else {
          await call(ctx, this.key, "DELETE", `/repos/${ctx.repo}/labels/${encodeURIComponent(label.name)}`);
...
// src/sections/misc.ts
var environmentsSection = {
  key: "environments",
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    for (const env of desiredRaw) {
      ...
      if (ctx.check) {
        ...
        if ("error" in probe) {
          result.drift.push(`environments[${name}]: missing - declared in the settings file but not on the repo; apply will create it`);
        } else {
          result.drift.push(...subsetDiff(settings, flattenEnvironment(probe.data), `environments[${name}]`));
        }
      } else {
...
var autolinksSection = {
  key: "autolinks",
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    const desired = desiredRaw;
    const live = await listAll(ctx, this.key, `/repos/${ctx.repo}/autolinks`);
    ...
    for (const autolink of desired) {
      ...
      const matches = existing !== undefined && subsetDiff(declaredFields, existing, "autolink").length === 0;
      if (matches) {
        continue;
      }
      if (ctx.check) {
        result.drift.push(existing ? `autolinks[${autolink.key_prefix}]: live settings differ from the settings file, and autolinks cannot be edited; apply will delete and recreate it` : `autolinks[${autolink.key_prefix}]: missing - declared in the settings file but not on the repo; apply will create it`);
        continue;
      }
      if (existing) {
    ...
    for (const autolink of live) {
      if (!declared.has(autolink.key_prefix)) {
        if (ctx.check) {
          result.drift.push(`autolinks[${autolink.key_prefix}]: undeclared - not in the settings file, so apply will DELETE it; add it to the settings file to keep it`);
        } else {
...
var actionsSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    if (routed.length > 0) {
      result.notes.push(`key(s) [${routed.join(", ")}] are not recognized by this action; they were sent verbatim to PUT /actions/permissions, where GitHub may ignore them - run mode: check to confirm they took effect, or remove them from the actions section of the settings file`);
    }
    if (ctx.check) {
      ...
      if (desired.selected_actions !== undefined) {
        const live = await call(ctx, this.key, "GET", `/repos/${ctx.repo}/actions/permissions/selected-actions`);
        result.drift.push(...subsetDiff(desired.selected_actions, live, "actions.selected"));
      }
      if (Object.keys(workflow).length > 0) {
        const live = await call(ctx, this.key, "GET", `/repos/${ctx.repo}/actions/permissions/workflow`);
    ...
    if (desired.selected_actions !== undefined) {
      await call(ctx, this.key, "PUT", `/repos/${ctx.repo}/actions/permissions/selected-actions`, desired.selected_actions);
...
var pagesSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    if (ctx.check) {
      if (!exists) {
        result.drift.push("pages: declared in the settings file but GitHub Pages is not enabled on the repo; apply will enable it");
      } else {
...
var milestonesSection = {
  ...
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    const desired = desiredRaw;
    const live = await listAll(ctx, this.key, `/repos/${ctx.repo}/milestones?state=all`);
    ...
    for (const milestone of desired) {
      ...
      const want = { ...milestone };
      if (!existing) {
        if (ctx.check) {
          result.drift.push(`milestones[${milestone.title}]: missing - declared in the settings file but not on the repo; apply will create it`);
        } else {
      ...
      if (drift.length > 0) {
        if (ctx.check) {
          ...
          await call(ctx, this.key, "PATCH", `/repos/${ctx.repo}/milestones/${existing.number}`, want);
    ...
    for (const milestone of live) {
      if (!declared.has(milestone.title)) {
        result.notes.push(`milestone "${milestone.title}" exists on the repo but is not declared in the settings file; left untouched - add it to the settings file to manage it, or delete it on GitHub (closing is not enough; closed milestones are still listed)`);
      }
    }
    return result;
  }
...
var collaboratorsSection = {
  ...
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    const desired = desiredRaw;
    const live = await listAll(ctx, this.key, `/repos/${ctx.repo}/collaborators?affiliation=direct`);
    ...
    for (const collaborator of desired) {
      ...
      const roleForPermission = { push: "write", pull: "read" };
      const wantRole = roleForPermission[permission] ?? permission;
      if (existing && (existing.role_name ?? "") === wantRole) {
        continue;
      }
      if (ctx.check) {
        result.drift.push(existing ? `collaborators[${collaborator.username}]: live role "${existing.role_name}" != declared "${wantRole}"; apply will set the declared permission` : `collaborators[${collaborator.username}]: missing - not a collaborator on the repo; apply will send an invitation with "${permission}"`);
      } else {
        const { username: _u, ...body } = collaborator;
        await call(ctx, this.key, "PUT", `/repos/${ctx.repo}/collaborators/${encodeURIComponent(collaborator.username)}`, { ...body, permission });
        result.changes.push(`${existing ? "updated" : "invited"} collaborator "${collaborator.username}" (${permission})`);
      }
    }
    for (const collaborator of live) {
      ...
      if (ctx.check) {
        result.drift.push(`collaborators[${collaborator.login}]: undeclared - not in the settings file, so apply will REMOVE them; add them to the settings file to keep their access`);
      } else {
        await call(ctx, this.key, "DELETE", `/repos/${ctx.repo}/collaborators/${encodeURIComponent(collaborator.login)}`);
...
var teamsSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    if ("error" in orgProbe) {
      ...
      result.notes.push(`teams: owner "${ctx.owner}" is a personal account, not an organization, so team access does not apply; section skipped - remove the teams section from the settings file to silence this note`);
      return result;
    }
    const roleForPermission = { push: "write", pull: "read" };
    for (const team of desired) {
      ...
      if (ctx.check) {
        ...
        if ("error" in probe) {
          result.drift.push(`teams[${team.name}]: no access to ${ctx.repo}; apply will grant "${team.permission ?? "push"}"`);
        } else {
          const permission = team.permission ?? "push";
          const wantRole = roleForPermission[permission] ?? permission;
          const liveRole = probe.data?.role_name ?? "";
          if (liveRole !== wantRole) {
            result.drift.push(`teams[${team.name}]: live role "${liveRole}" != declared "${wantRole}"; apply will set the declared permission`);
          }
        }
      } else {
...
var repositorySection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    if (ctx.check) {
      const live = await call(ctx, this.key, "GET", `/repos/${ctx.repo}`);
      result.drift.push(...subsetDiff(patch, live, "repository"));
      if ("topics" in desired) {
        result.drift.push(...subsetDiff(normalizeTopics(desired.topics).sort(), (live.topics ?? []).slice().sort(), "repository.topics"));
      }
      if ("enable_vulnerability_alerts" in desired) {
        ...
        if (enabled !== Boolean(desired.enable_vulnerability_alerts)) {
          result.drift.push(`repository.enable_vulnerability_alerts: declared ${desired.enable_vulnerability_alerts} != live ${enabled}; apply will set the declared value`);
        }
      }
      if ("enable_automated_security_fixes" in desired) {
        const probe = await ctx.api.tryRequest("GET", `/repos/${ctx.repo}/automated-security-fixes`);
        if ("error" in probe && probe.error.status !== 404) {
          throwFor(this.key, "GET", `/repos/${ctx.repo}/automated-security-fixes`, probe.error);
        }
        const enabled = !("error" in probe) && (probe.data === null || probe.data?.enabled !== false);
        if (enabled !== Boolean(desired.enable_automated_security_fixes)) {
          result.drift.push(`repository.enable_automated_security_fixes: declared ${desired.enable_automated_security_fixes} != live ${enabled}; apply will set the declared value`);
        }
      }
      return result;
    }
...
// src/sections/rulesets.ts
var rulesetsSection = {
  key: "rulesets",
  async run(ctx, desiredRaw) {
    const result = emptyResult();
    const desired = desiredRaw.map(normalizeRuleset);
    const summaries = await listAll(ctx, this.key, `/repos/${ctx.repo}/rulesets`);
    ...
    for (const ruleset of desired) {
      ...
      if (id === undefined) {
        if (ctx.check) {
          result.drift.push(`rulesets[${ruleset.name}]: missing - declared in the settings file but not on the repo; apply will create it`);
        } else {
    ...
    for (const live of repoRulesets) {
      if (!declaredNames.has(live.name)) {
        result.notes.push(`ruleset "${live.name}" exists on the repo but is not declared in the settings file; left untouched - add it to the settings file to manage it, or delete it in the repo's GitHub settings`);
      }
    }
    return result;
  }
...
async function runForRepo(api, opts, io) {
  ...
  if (!ctx.check && opts.onMissingPermission === "fail") {
    ...
    for (const section of active) {
      try {
        await section.run({ ...ctx, check: true }, settings[section.key]);
      } catch (error) {
  ...
  for (const section of SECTIONS) {
    ...
    let result2;
    try {
      result2 = await section.run(ctx, desired);
    } catch (error) {
      if (error instanceof PermissionDenied) {
        ...
        io.annotate("error", `${L}${section.key}: not applied${required ? " (listed in required-sections, so this fails the run)" : ""} - ${error.detail}`);
        outcomes.push({ key: section.key, status: "failed", detail: [error.detail] });
        failed = true;
        continue;
      }
      const message = error instanceof Error ? error.message : String(error);
      const annotated = message.startsWith(`${section.key}:`) ? message : `${section.key}: ${message}`;
      io.annotate("error", `${L}${annotated}`);
      outcomes.push({ key: section.key, status: "failed", detail: [annotated] });
      failed = true;
      continue;
    }
    for (const note of result2.notes) {
      io.annotate("notice", `${L}${section.key}: ${note}`);
    }
    if (ctx.check) {
      if (result2.drift.length > 0) {
        drifted = true;
        for (const line of result2.drift) {
          io.log(`${L}drift: ${line}`);
        }
        outcomes.push({ key: section.key, status: "drift", detail: result2.drift });
      } else {
        outcomes.push({ key: section.key, status: "clean", detail: result2.notes });
      }
    } else {
      for (const line of result2.changes) {
        io.log(`${L}${section.key}: ${line}`);
      }
      outcomes.push({
        key: section.key,
        status: "applied",
        detail: result2.changes.length > 0 ? result2.changes : ["no changes needed"]
      });
    }
  }
  const result = failed ? "failed" : ctx.check ? drifted ? "drift" : partial ? "partial" : "clean" : partial ? "partial" : "applied";
  return {
    repo: opts.repo,
    result,
    outcomes,
...
function worstOf(results, check) {
  const ranks = ["failed", "drift", "partial", "skipped", "applied", "clean"];
    ...
    for (const entry of readdirSync(reposDir).sort()) {
      ...
      if (statSync(entryPath).isDirectory()) {
        for (const inner of readdirSync(entryPath).sort()) {
          ...
          const bad2 = addTarget(slug, innerPath);
          if (bad2) {
            return { error: bad2 };
          }
        }
...
async function discoverRepos(api) {
  let repos;
  try {
    repos = await api.list("/user/repos?affiliation=owner");
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return {
...
function dedupeTargets(central, remote, notice) {
  ...
  for (const target of remote) {
    ...
    if (centralKeys.has(target.slug.toLowerCase())) {
      notice(`${target.slug}: using the central file ${winner?.origin}; the ${target.origin} entry for the same repository is ignored`);
      continue;
    }
    out.push(target);
  }
...
function writeSummary(outcomes, mode) {
...
function writeMultiSummary(targets, mode) {
  const file = process.env.GITHUB_STEP_SUMMARY;
  if (!file) {
    return;
  }
  ...
  for (const target of targets) {
    lines.push(`| ${target.slug} | ${target.source} | :${STATUS_ICON[target.result]}: ${target.result} |`);
  }
  for (const target of targets) {
...
async function runMulti(api, cfg, io) {
  const none = [];
  let defaults = {};
  if (cfg.defaultsFile) {
    try {
      defaults = $parse(readFileSync(cfg.defaultsFile, "utf8")) ?? {};
    } catch (error) {
      return {
  ...
  if (cfg.reposInput) {
    ...
    if (parsed.discover) {
      ...
      for (const archived of discovered.archivedSkipped) {
        io.annotate("notice", `${archived}: skipped - the repository is archived, and settings writes fail on archived repositories. Unarchive it to manage it`);
      }
      slugs = discovered.slugs;
      origin = 'repos: "*" discovery';
    }
    remote = slugs.map((slug) => ({ slug, source: "remote", origin }));
  }
    ...
    try {
      ...
      if (target.source === "central") {
        ...
        try {
          ...
          failTarget(`cannot read settings from ${sourceLabel}: ${String(error)}. Fix the file, or delete it to stop managing this repository`);
          continue;
        }
      } else {
        ...
        if ("error" in gate) {
          failTarget(isPermissionError(gate.error) ? `the token was denied GET /repos/${target.slug}: ${gate.error.status} ${gate.error.message}. Grant the PAT access to this repository, or remove it from the "repos" input` : `GET /repos/${target.slug} failed: ${gate.error.status} ${gate.error.message}. This is not a permission problem; re-run the workflow, and retry later if it persists`);
          continue;
        }
        const file = await api.getRepoFile(target.slug, ".github/settings.yml");
        if ("missing" in file) {
          io.annotate("notice", `${target.slug}: skipped - the repository has no .github/settings.yml on its default branch. Add the file to manage it, or remove ${target.slug} from the "repos" input`);
          results.push({
            slug: target.slug,
            source: target.source,
            origin: target.origin,
        ...
        if ("error" in file) {
          failTarget(isPermissionError(file.error) ? `the token was denied reading ${sourceLabel}: ${file.error.status} ${file.error.message}. Grant the PAT Contents (read) on this repository, or remove it from the "repos" input` : `reading ${sourceLabel} failed: ${file.error.status} ${file.error.message}. This is not a permission problem; re-run the workflow, and retry later if it persists`);
          continue;
        }
        raw = file.content;
      }
      ...
      for (const key of disabled) {
        io.annotate("notice", `${target.slug}: section "${key}" is set to null in ${sourceLabel}, which opts this repository out of that defaults-file section`);
      }
      ...
      const run = await runForRepo(api, {
        repo: target.slug,
        settings,
        mode: cfg.mode,
      ...
      if (run.preflightDenied.length > 0) {
        ...
        io.annotate("error", `${target.slug}: preflight failed: the token cannot access ${run.preflightDenied.length} section(s), so nothing was applied to this repository. Grant the permissions named above, or set on-missing-permission: warn`);
      }
      results.push({
        slug: target.slug,
        source: target.source,
...
async function run(overrides) {
  ...
  const io = { annotate, log: (line) => console.log(line) };
  const token = input("token") || process.env.GITHUB_TOKEN || "";
  if (!token) {
    return fail('cannot call the GitHub API: no token was provided. Set the "token" input on the action step (or export GITHUB_TOKEN)');
  }
  const mode = input("mode") || "apply";
  if (mode !== "apply" && mode !== "check") {
    return fail(`the "mode" input is "${mode}", which is not a supported mode. Set it to "apply" (mutate settings) or "check" (report drift only)`);
  }
  const onMissingPermission = input("on-missing-permission") || "fail";
  if (onMissingPermission !== "fail" && onMissingPermission !== "warn") {
    return fail(`the "on-missing-permission" input is "${onMissingPermission}", which is not a supported policy. Set it to "fail" (default) or "warn" (skip sections the token cannot touch)`);
  }
  const requiredSections = new Set(input("required-sections").split(",").map((s) => s.trim()).filter(Boolean));
  const onlySections = new Set(input("sections").split(",").map((s) => s.trim()).filter(Boolean));
  const knownSections = new Set(SECTION_KEYS);
  for (const name of [...requiredSections, ...onlySections]) {
    if (!knownSections.has(name)) {
      return fail(`unknown section "${name}" in the "sections" or "required-sections" input; it matches none of: ${SECTION_KEYS.join(", ")}. Fix the name in the workflow's input list`);
    }
  }
  ...
  if (reposInput || reposDir) {
    if (input("repository")) {
      return fail('the "repository" input cannot be combined with "repos" or "repos-dir"; multi-repo targets come from those inputs. Remove "repository", or remove the multi-repo inputs to stay in single-repo mode');
    }
    if (settingsFile !== ".github/settings.yml") {
      return fail(`the "settings-file" input cannot be combined with "repos" or "repos-dir": central targets are read from repos-dir files and remote targets from each repository's own .github/settings.yml. Remove the settings-file override`);
    }
    const adminOwner = (process.env.GITHUB_REPOSITORY ?? "").split("/")[0] ?? "";
    const { fatal, targets } = await runMulti(api, {
      reposDir,
      reposInput,
      defaultsFile,
      adminOwner,
    ...
    setOutput("repos-result", JSON.stringify(Object.fromEntries(targets.map((t) => [
      t.slug,
      { result: t.result, source: t.source, skippedSections: t.skippedSections }
    ]))));
    setOutput("skipped-sections", [...new Set(targets.flatMap((t) => t.skippedSections))].join(","));
  ...
  if (!SLUG_RE.test(repo)) {
    return fail(`cannot target a repository: "${repo}" is not an owner/name slug. Set the "repository" input (or GITHUB_REPOSITORY) to a value like "octocat/hello-world"`);
  }
  let settings;
  try {
    settings = $parse(readFileSync(settingsFile, "utf8")) ?? {};
  } catch (error) {
    return fail(`cannot read settings from ${settingsFile}: ${String(error)}. Check that the file exists at that path (set the "settings-file" input if it lives elsewhere) and is valid YAML`);
  }
  const invalid = validateSettingsDoc(settings, settingsFile, onlySections, io);
  if (invalid) {
    return fail(invalid);
  }
  const result = await runForRepo(api, { repo, settings, mode, onMissingPermission, requiredSections, onlySections, label: "" }, io);
  if (result.preflightDenied.length > 0) {
    return fail(`preflight failed: the token cannot access ${result.preflightDenied.length} section(s), so nothing was applied. Grant the permissions named above, or set on-missing-permission: warn to skip those sections`);
  }
...
var invokedDirectly = process.argv[1]?.endsWith("main.ts") || process.argv[1]?.endsWith("index.js");
if (invokedDirectly) {
  run().then((code) => process.exit(code), (error) => {
    annotate("error", `repo-settings-as-code stopped unexpectedly: ${String(error)}. Re-run the workflow; if it recurs, report a bug with this log attached`);
    process.exit(1);
  });
}
export {
