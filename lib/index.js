...
var autolinksSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const autolink of desired) {
      ...
      const matches = existing !== undefined && existing.url_template === autolink.url_template && existing.is_alphanumeric === (autolink.is_alphanumeric ?? true);
      if (matches) {
        continue;
      }
      if (ctx.check) {
...
var milestonesSection = {
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const milestone of desired) {
      ...
      const want = { title: milestone.title };
      ...
      const descriptionDrift = milestone.description !== undefined && (existing.description ?? "") !== milestone.description;
  ...
  async run(ctx, desiredRaw) {
    ...
    for (const collaborator of desired) {
      ...
      if (ctx.check) {
        ...
        await call(ctx, this.key, "PUT", `/repos/${ctx.repo}/collaborators/${encodeURIComponent(collaborator.username)}`, { permission });
...
async function run() {
  ...
  if (unknownKeys.length > 0) {
    return fail(`unknown top-level section(s) in ${settingsFile}: ${unknownKeys.join(", ")} (known: ${SECTION_KEYS.join(", ")})`);
  }
